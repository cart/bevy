...
#[derive(Component, Clone, Debug, Deref, DerefMut, Reflect, PartialEq, Eq, From, FromTemplate)]
#[reflect(Component, Clone)]
pub struct AnimationGraphHandle(#[template] pub Handle<AnimationGraph>);
            ...
            | AssetEvent::LoadedWithDependencies { id } => {
                ...
                for graph_id in graph_ids {
                    if animation_target_graphs_rebuilt_this_frame.insert(*graph_id)
                        ...
                        && let Ok(animation_graph) = animation_graphs.get(*graph_id)
                    {
                        threaded_animation_graph
                            .rebuild_target_subgraphs(&animation_graph.graph, &animation_clips);
                    }
        ...
        for node in graph.node_weights() {
            let animation_clip = match &node.node_type {
                AnimationNodeType::Clip(animation_clip_id) => {
                    ...
                    let Ok(animation_clip) = clips.get(animation_clip_id) else {
                        continue;
                    };
                    animation_clip
                }
        ...
        for node_index in &self.threaded_graph {
            ...
            match &node.node_type {
                AnimationNodeType::Clip(clip_handle) => {
                    ...
                    if clips
                        .get(clip_handle)
                        .is_ok_and(|clip| clip.is_relevant_to_target(animation_target_id))
                    {
