    ...
    fn build(&self, app: &mut App) {
        ...
        if !app.world().is_resource_added::<TonemappingLuts>() {
            ...
            let tonemapping_luts = {
                let blender_filmic = app.world_mut().spawn_asset(setup_tonemapping_lut_image(
                    include_bytes!("luts/Blender_-11_12.ktx2"),
                    ImageType::Extension("ktx2"),
                ));
                let agx = app.world_mut().spawn_asset(setup_tonemapping_lut_image(
                    include_bytes!("luts/AgX-default_contrast.ktx2"),
                    ImageType::Extension("ktx2"),
                ));
                let tony_mc_mapface = app.world_mut().spawn_asset(setup_tonemapping_lut_image(
