use crate::core::{ArchetypeComponentId, ArchetypeId, Component, Mut, SparseSet, World};
use std::{
    any::TypeId,
    cell::RefCell,
    ops::{Deref, DerefMut},
...
/// Exposes safe "interior mutability" on a World. Attempting to access World in a way that violates
/// Rust's mutability rules will panic
...
impl<'w> WorldCell<'w> {
    ...
    pub fn get_resource_mut<T: Component>(&self) -> Option<WorldBorrowMut<'_, T>> {
        ...
        Some(WorldBorrowMut::new(
            ...
            unsafe {
                self.world
                    .get_resource_mut_unchecked_with_id(component_id)?
            },
            archetype_component_id,
            self.access.clone(),
        ))
    ...
    pub fn get_non_send_mut<T: 'static>(&self) -> Option<WorldBorrowMut<'_, T>> {
        ...
        Some(WorldBorrowMut::new(
            ...
            unsafe {
                self.world
                    .get_non_send_mut_unchecked_with_id(component_id)?
            },
            archetype_component_id,
            self.access.clone(),
        ))
...
mod tests {
    ...
    fn world_cell() {
        ...
        }
        {
            ...
            let b = cell.get_resource::<f32>().unwrap();
            assert_eq!(
                1.0, *b,
