...
pub struct Trigger<'w, E: Event, B: Bundle = ()> {
    event: &'w mut E,
    trigger: ObserverTrigger<E::Target>,
    _marker: PhantomData<B>,
}

impl<'w, E: Event, B: Bundle> Trigger<'w, E, B> {
    /// Creates a new trigger for the given event and observer information.
    pub fn new(event: &'w mut E, trigger: ObserverTrigger<E::Target>) -> Self {
        Self {
            event,
            trigger,
            _marker: PhantomData,
...
impl<'w, E: Event<Target = Entity>, B: Bundle> Trigger<'w, E, B> {
...
/// Collection of [`ObserverRunner`] for [`Observer`] registered to a particular trigger.
#[derive(Default, Debug)]
pub struct CachedEntityObservers {
    // Observers listening for any time this trigger is fired
    map: ObserverMap<Entity>,
    // Observers listening for this trigger fired at a specific component
    component_observers: HashMap<ComponentId, CachedCompoentEntityObservers>,
    // Observers listening for this trigger fired at a specific entity
    entity_observers: EntityHashMap<Entity, ObserverMap<Entity>>,
}
...
pub struct Observers {
    ...
    on_add: CachedEntityObservers,
    on_insert: CachedEntityObservers,
    on_remove: CachedEntityObservers,
    // Map from trigger type to set of observers
    global_cache: HashMap<ComponentId, CachedObservers>,
    // Map from trigger type to set of observers
    entity_cache: HashMap<ComponentId, CachedEntityObservers>,
}

impl Observers {
    pub(crate) fn get_entity_observers(
        &mut self,
        event_type: ComponentId,
    ) -> &mut CachedEntityObservers {
        match event_type {
            ON_ADD => &mut self.on_add,
            ON_INSERT => &mut self.on_insert,
            ON_REMOVE => &mut self.on_remove,
            _ => self.entity_cache.entry(event_type).or_default(),
        }
    }

    pub(crate) fn try_get_entity_observers(
        &self,
        event_type: ComponentId,
    ) -> Option<&CachedEntityObservers> {
        match event_type {
            ON_ADD => Some(&self.on_add),
            ON_INSERT => Some(&self.on_insert),
            ON_REMOVE => Some(&self.on_remove),
            _ => self.entity_cache.get(&event_type),
        }
    }

    pub(crate) fn get_observers(&mut self, event_type: ComponentId) -> &mut CachedObservers {
        self.global_cache.entry(event_type).or_default()
    }

    pub(crate) fn try_get_observers(&self, event_type: ComponentId) -> Option<&CachedObservers> {
        self.global_cache.get(&event_type)
    }

    /// This will run the observers of the given `event_type`, targeting the given `entity` and `components`.
    pub(crate) fn invoke_entity_target<E: Event<Target = Entity>>(
        ...
        let (mut world, observers) = unsafe {
            ...
            let Some(observers) = observers.try_get_entity_observers(event_type) else {
                return;
            };
        ...
        let mut trigger_observer = |(&observer, runner): (&Entity, &ObserverRunner<E::Target>)| {
            (runner)(
                world.reborrow(),
                ObserverTrigger {
                    observer,
        ...
        for component in components {
            if let Some(component_observers) = observers.component_observers.get(&component) {
                component_observers
                    .map
                    .iter()
    ...
    /// This will run the observers of the given `event_type`, targeting the given `entity` and `components`.
    pub(crate) fn invoke<E: Event<Target = ()>>(
        mut world: DeferredWorld,
        event_type: ComponentId,
        data: &mut E,
        components: impl Iterator<Item = ComponentId>,
        ...
        components.for_each(|id| {
            if let Some(component_observers) = observers.component_observers.get(&id) {
                component_observers.iter().for_each(&mut trigger_observer);
            }
        });
    }
...
impl World {
    ...
    pub fn trigger_entities<E: Event<Target = Entity>, I: IntoIterator<Item = Entity>>(
        &mut self,
        event: E,
        entities: impl AsIter<Entity> + Send + Sync + 'static,
    ) {
        TriggerEventWithTargets {
            event,
            components: std::iter::empty(),
            targets: entities,
        }
        ...
        for &event_type in &descriptor.events {
            ...
            if descriptor.components.is_empty() {
                ...
                for &component in &descriptor.components {
                    ...
                    observers.insert(observer_entity, observer_state.runner);
                }
            }
        }
    }
    ...
    /// Register an observer to the cache, called when an observer is created
    pub(crate) fn register_entity_observer(&mut self, observer_entity: Entity) {
        // SAFETY: References do not alias.
        let (observer_state, archetypes, observers) = unsafe {
            let observer_state: *const ObserverState<Entity> =
                self.get::<ObserverState<Entity>>(observer_entity).unwrap();
            ...
            for watched_entity in &(*observer_state).descriptor.entities {
                ...
                observed_by.observer_entities.push(observer_entity);
            }
            (&*observer_state, &mut self.archetypes, &mut self.observers)
        };
        let descriptor = &observer_state.descriptor;
        ...
        for &event_type in &descriptor.events {
            let cache = observers.get_entity_observers(event_type);
                    ...
                    let observers =
                        cache
                            ...
                            .or_insert_with(|| {
                                ...
                                CachedCompoentEntityObservers::default()
                            });
    ...
    pub(crate) fn unregister_entity_observer(
        &mut self,
        entity: Entity,
...
mod tests {
    ...
    use crate::observer::{Observer, ObserverDescriptor, ObserverState};
    use crate::prelude::*;
    ...
    fn observer_entity_routing() {
        ...
        // and therefore does not automatically flush.
        world.flush();
        world.trigger_entities(EventA, entity);
        world.flush();
        assert_eq!(2, world.resource::<R>().0);
    }
