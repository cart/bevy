...
impl Default for ParallelSystemStageExecutor {
    fn default() -> Self {
        Self {
            ...
            last_archetypes_generation: ArchetypesGeneration(u32::MAX), // MAX forces prepare to run the first time
        }
    }
}
...
impl ParallelSystemStageExecutor {
    ...
    pub fn prepare_to_next_thread_local(
        ...
        let archetypes_generation_changed =
            self.last_archetypes_generation != world.archetypes().generation();
...
impl SystemStageExecutor for ParallelSystemStageExecutor {
    fn execute_stage(
        ...
        let start_archetypes_generation = world.archetypes().generation();
        ...
        if start_archetypes_generation == world.archetypes().generation() {
            self.last_archetypes_generation = world.archetypes().generation();
        }
    }
}
