use bevy_asset::{AssetCommands, Handle, HandleTemplate, RenderAssetUsages};
...
impl EnvironmentMapLight {
    ...
    pub fn solid_color(commands: &mut Commands, color: impl Into<Color>) -> Self {
        let color = color.into();
        Self::hemispherical_gradient(commands, color, color, color)
    }
    ...
    pub fn hemispherical_gradient(
        ...
        let handle = commands.spawn_asset(Self::hemispherical_gradient_cubemap(
            top_color.into(),
            mid_color.into(),
            bottom_color.into(),
        ));
