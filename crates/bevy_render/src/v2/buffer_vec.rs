...
use crate::renderer::{
    BufferId, BufferInfo, BufferMapMode, BufferUsage, RenderContext, RenderResources2,
};
...
impl<T: Byteable> BufferVec<T> {
    ...
    pub fn reserve(&mut self, capacity: usize, render_resources: &RenderResources2) {
    ...
    pub fn reserve_and_clear(&mut self, capacity: usize, render_resources: &RenderResources2) {
        self.clear();
        self.reserve(capacity, render_resources);
    }

    pub fn write_to_staging_buffer(&self, render_resources: &RenderResources2) {
