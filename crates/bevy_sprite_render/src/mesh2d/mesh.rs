...
fn extract_2d_mesh(
    ...
    // yet loaded.
    let Some(material_instance) = render_material_instances.get(&main_entity) else {
        reextract_entities.insert(main_entity);
        return;
    };
    let Some(mesh_material_binding_id) = render_material_bindings
        .get(&material_instance.asset_entity)
        .copied()
    else {
        reextract_entities.insert(main_entity);
        return;
