...
use bevy_asset::{
    Asset, AssetCommands, AssetEvent, AssetEventSystems, AssetId, AssetServer, Handle,
};
...
impl<M: Asset> SpriteMaterialCache<M> {
    ...
    fn get_or_insert_with(
        ...
        let maybe_handle = bucket
            ...
            .and_then(|(_, id)| assets.get_id_handle(*id));

        match maybe_handle {
            Some(handle) => handle,
            None => {
                let handle = assets.add(get());
...
fn add_material(
    ...
    for (entity, sprite, anchor) in sprites {
        let handle = cached_materials.get_or_insert_with(sprite, *anchor, &assets, || {
...
fn make_sprite_mesh_material(
    ...
    if let Some(texture_atlas) = &sprite.texture_atlas
        && let Ok(texture_atlas_layout) = texture_atlas_layouts.get(&texture_atlas.layout)
    {
