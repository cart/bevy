...
use bevy_asset::{AssetEvent, AssetEventSystems, AssetId, AssetReference, AssetServer};
...
pub fn extract_uinode_images(
    mut commands: Commands,
    extracted_uinodes: ResMut<ExtractedUiNodes>,
    texture_atlases: Extract<Query<&TextureAtlasLayout>>,
    uinode_query: Extract<
        Query<(
            Entity,
            &ComputedNode,
            ...
            for item_index in 0..ui_phase.items.len() {
                ...
                if batch_image_handle.is_none()
                    ...
                    || (batch_image_handle != Some(default_image.id())
                        && extracted_uinode.image != default_image.id()
                ...
                } else if batch_image_handle == Some(default_image.id())
                    && extracted_uinode.image != default_image.id()
                {
                ...
                match &extracted_uinode.item {
                    ExtractedUiItem::Node {
                        ...
                        let mut flags = if extracted_uinode.image != default_image.id() {
