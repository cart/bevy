...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let cube = commands.spawn_asset(Mesh::from(Cuboid::new(0.5, 0.5, 0.5)));
    ...
    for x in -1..2 {
        for z in -1..2 {
            let material = commands.spawn_asset(StandardMaterial::from(Color::from(hsla)));
            commands.spawn((
                Mesh3d(cube.clone()),
...
fn animate_materials(
    ...
    for material_handle in material_handles.iter() {
        if let Ok(mut material) = materials.get_mut(material_handle.entity())
