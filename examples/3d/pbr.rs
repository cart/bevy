...
fn environment_map_load_finish(
    ...
    if let Ok(environment_map) = environment_maps.get_single() {
        if asset_server.get_load_state(&environment_map.diffuse_map) == AssetLoadState::Loaded
            && asset_server.get_load_state(&environment_map.specular_map) == AssetLoadState::Loaded
        {
