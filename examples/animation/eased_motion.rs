...
fn setup(mut commands: Commands) {
    ...
    } = AnimationInfo::create(&mut commands);
    ...
    let mesh = commands.spawn_asset(Mesh::from(Cuboid::from_length(2.0)));
    let material = commands.spawn_asset(StandardMaterial::from(Color::from(ORANGE)));
    ...
    // Ground plane
    let mesh = commands.spawn_asset(Mesh::from(Plane3d::default().mesh().size(50., 50.)));
    let material = commands.spawn_asset(StandardMaterial::from(Color::from(SILVER)));
...
impl AnimationInfo {
    ...
    fn create(commands: &mut Commands) -> AnimationInfo {
        ...
        let animation_clip_handle = commands.spawn_asset(animation_clip);

        // Create an animation graph with that clip.
        let (animation_graph, animation_node_index) =
            AnimationGraph::from_clip(animation_clip_handle);
        let animation_graph_handle = commands.spawn_asset(animation_graph);
