...
fn spawn_view_model(mut commands: Commands) {
    let arm = commands.spawn_asset(Mesh::from(Cuboid::new(0.1, 0.1, 0.5)));
    let arm_material =
        commands.spawn_asset(StandardMaterial::from(Color::from(tailwind::TEAL_200)));
...
fn spawn_world_model(mut commands: Commands) {
    let floor = commands.spawn_asset(Mesh::from(Plane3d::new(Vec3::Y, Vec2::splat(10.0))));
    let cube = commands.spawn_asset(Mesh::from(Cuboid::new(2.0, 0.5, 1.0)));
